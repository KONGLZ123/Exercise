...
void ServiceGenerator::GenerateMethodSignatures(StubOrNon stub_or_non, io::Printer* printer)
{
  for (int i = 0; i < descriptor_->method_count(); ++i) {
	...
	if (stub_or_non == NON_STUB) {
	  ...
	  printer->Print(sub_vars,
		...
		"$virtual$void $name$(const $input_type$& reuqest,\n"
...
void ServiceGenerator::GenerateImplementation(io::Printer* printer)
{
  ...
  printer->Print(vars_,
	...
	"  : channel_(channel__), owns_channel(false) {}\n"
...
void ServiceGenerator::GenerateStubMethods(io::Printer* printer) 
{
  for (int i = 0; i < descriptor_->method_count(); ++i) {
	...
	printer->Print(sub_vars,
	  "void $classname$_Stub::$name$(const $input_type$& reuqest,\n"
	  "                              const ::std::function<void(const $output_typedef$Ptr&)>& done) {\n"
